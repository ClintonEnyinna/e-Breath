...
class ManualConnection extends Component {
  ...
  onReadData(data) {
    if (
      ...
      this.receivedData = this.receivedData + payload;

      if (testStr) {
        this.setState({output: this.receivedData});

        const payloadFloat = parseFloat(this.receivedData);
        if (Number.isNaN(payloadFloat)) {
          console.log('not a number');
        } else {
          let prevData = this.data.slice(0);
          prevData[this.i] = payloadFloat;

          if (this.i === prevData.length - 1) {
            this.i = 0;
          } else {
            prevData[this.i + 1] = undefined;
            this.i++;
          }
